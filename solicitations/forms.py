...
from solicitations.models import EmailSettings, GitHubWorkflow,UserOEMCustomization,UserEmailConfig
...
class EmailConfigForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['email_use_tls'].widget.attrs.update({'class': 'form-check-input'})
        self.fields['save_to_sent_folder'].widget.attrs.update({'class': 'form-check-input'})
        ...
        # Add help text
        self.fields['email_interval_seconds'].help_text = 'Time delay between sending emails to different recipients (3-240 seconds)'
    ...
    def clean_email_interval_seconds(self):
        ...
        if interval is not None:
            ...
            if interval > 240:
                raise forms.ValidationError("Maximum interval is 4 minutes (240 seconds)")
        return interval
    ...
    def clean(self):
        ...
        if new_password1:
            ...
            if new_password1.lower() in [self.user.username.lower(), self.user.email.lower()]:
                raise forms.ValidationError('Password cannot be similar to your username or email.')
